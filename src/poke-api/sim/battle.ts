...
export class Battle {
	...
	constructor(options: BattleOptions) {
		...
		this.format = format;
		this.dex = Dex; // Asegúrate de que `Dex` esté correctamente importado
		this.gen = this.dex.gen; // Ahora `this.dex` no será undefined
	...
	add(...parts: (Part | (() => { side: SideID, secret: string, shared: string }))[]) {
		...
		for (const part of parts) {
			if (typeof part === 'function') {
				...
				secret.push(String(part));
				shared.push(String(part));
			}
		}
